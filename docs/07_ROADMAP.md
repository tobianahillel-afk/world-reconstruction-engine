...
- The Strategy Router is implemented in L4 before the heavy L13 escalation engines. It selects FAST/STANDARD/ESCALATED routes but never weakens the final acceptance policy/evidence standard.
- Absolute world placement is optional until evidence exists. L9 adds GPS/GCP/reference-fragment factors rather than requiring GPS during reconstruction. A reference anchor may resolve similarity scale as well as pose; the transform model remains explicit.
- L15 follows dense reconstruction so multi-epoch point-cloud change analysis can reuse stable dense products where needed; temporal models may also reference sparse/structural geometry. Texture itself is not evidence required for temporal change.
- Metric 3D-change claims require compatible frames and sufficiently resolved scale; otherwise the result remains unresolved.
- L16 validates both absolute anchoring and the L15 temporal engine rather than allowing either capability to rely only on local unit tests.
